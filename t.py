    ...
    try:
        ...
        if op.type == 25:
           ...
           if msg.contentType == 0:
              if text is None:
                 return
              if textlower == 'loginsb':
                 try:
                    a = API("LosAngeles", "v2")
                    qrResult = a._lineqr("DESKTOPWIN\t7.5.0\tWindows\t10\t20H""DESKTOPWIN\t7.5.0\tWindows\t10\t20H", "Mozilla/5.0 (X11; CrOS x86_64 14268.67.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.111 Safari/537.36")
                    link = f'{qrResult["QrImage"]}'
                    try:
                      cline.sendImageWithURL(to, str(link))
                      pinResult = a._linepin(qrResult['Key'], qrResult['Session'], "DESKTOPWIN\t7.5.0\tWindows\t10\t20H", "Mozilla/5.0 (X11; CrOS x86_64 14268.67.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.111 Safari/537.36")
                      cline.sendMessage(sender, f'รหัสเข้าสู่ระบบ : {pinResult["Pincode"]}')
                    except Exception as e:
                      cline.sendMessage(to, str(e))
                 except:pass
